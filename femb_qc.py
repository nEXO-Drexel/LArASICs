...
class FEMB_QC:
    ...
    def FEMB_CHK_ACQ(self, testcode = 0, rs=None):
...
#            time.sleep(30)
        ...
        if len(self.raw_data) != 0: 
            for a_femb_data in self.raw_data:
                ...
                if (map_r != None):
                    ...
                    if (self.avg_cnt)<=1:
                        #print('in FEMB_PLOT: self.avg_cnt<=1')
                        ...
                        print('in FEMB_PLOT: self.avg_cnt is not <=1')
    ...
    def FEMB_CHKOUT_Input(self, crateno=0, slotno=0):
        #FEMBlist = self.FEMB_INDEX_LOAD()
        #print('running FEMB_CHKOUT_Input')
        FEMB_infos = []
        env = self.env
        for i in range(4):
    ...
    def FEMB_CHKOUT(self, FEMB_infos, pwr_int_f = False, testcode = 0, ana_flg=True, reg_settings_dict=None):
        ...
        pwr_qcs = []
        self.CLS.pwr_int_f = pwr_int_f
        qc_data = self.FEMB_CHK_ACQ(testcode = testcode, rs=reg_settings_dict)
        if ana_flg:
