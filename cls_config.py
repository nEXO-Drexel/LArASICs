# -*- coding: utf-8 -*-
"""
File Name: cls_config.py
Author: GSS/beckert
Mail: gao.hillhill@gmail.com/be348@drexel.edu
Description: 
Created Time: 3/20/2019 4:50:34 PM by GSS gao.hillhill@gmail.com
...
class CLS_CONFIG:
    ...
    def WIBs_SCAN(self ):
        print ("WIBs_SCAN...Finding available WIBs starts...")
        active_wibs = []
        ...
        for wib_ip in self.WIB_IPs:
            ...
            self.UDP.write_reg_wib_checked(20, 0x00) #enable data stream 
        ...
        if len(active_wibs) == 0:
            print ("No WIB is available, please check power for WIB is ON! Exit Anyway!")
            sys.exit()
        else:
...
#                reg35 = self.UDP.read_reg_wib(0x25) #0x25 is reg37 and is reserved, correction for frame error counter register below
                reg35 = self.UDP.read_reg_wib(0x23)
    ...
    def FEMBs_SCAN(self):
        print ("FEMBs_SCAN...Finding available FEMBs starts...")
        self.err_code = "" 
        for wib_ip in self.WIB_IPs:
...
#            self.WIB_CLKCMD_cs(wib_ip )# choose clock source
    ...
    def CE_MON_CFG(self, \
                   ...
                   pls_gap = 500, pls_dly = 10, mon_cs=1, \
                   data_cs = 0, \
                   sts=0, snc=0, sg0=0, sg1=1, st0=1, st1=1, sdf=1, \
        ...
        for wib_ip in list(self.act_fembs.keys()):
            ...
            #for femb_addr in [0]:
                if self.act_fembs[wib_ip][femb_addr] == True:
                    self.UDP.UDP_IP = wib_ip
                    print ("UDP.UDP_IP = ",self.UDP.UDP_IP )
        ...
        for wib_ip in list(self.act_fembs.keys()):
            for femb_addr in range(4):
                if self.act_fembs[wib_ip][femb_addr] == True:
                    ...
                    #reg0: reset the entire system
                    self.UDP.write_reg_femb(femb_addr,  0, 1)
                    time.sleep(0.001)

                    # reg5:set pulse amplitude(6bit fpgadac_v->int), delay(10ns*pls_dly), and period (500ns*(pls_gap+1))
                    self.UDP.write_reg_femb_checked (femb_addr,  5, reg_5_value)

                    # reg16: bit0=1-enable FPGA-DAC; bit1=1-enable ASIC-DAC; bit8=0/1-select analog pulse source going to FE-ASIC- FPGA-DAC on FM/analog pulse from the WIB
                    self.UDP.write_reg_femb_checked (femb_addr, 16, tp_sel&0x0000ffff)

                    #reg18: bit0=0 enables internal pulse generator, bit1=0 allows test pulse to be received by external timing control interface
                    self.UDP.write_reg_femb_checked (femb_addr, 18, pls_cs_value)
                    ...
                    for j in range(len(fe_regs)):
                        if (fe_regs[j] != fe_rb_regs[j]) and (data_cs == 0 ):
                            ...
                            if  fid in self.err_code:  
                                ...
                                self.err_code = self.err_code[0:t] + spi_err + self.err_code[t:]
                    #enable data stream to WIB and reset transceiver
                    self.UDP.write_reg_femb_checked (femb_addr, 9, 9) #9=0b1001

                    bl_mean = 0
                    bl_rms = 0
                    if (self.fe_monflg):
                        print('self.fe_monflg = True in CE_CHKCFG')
                        bl = []
                        for i in range(10):
                            adc_v = self.FEMB_MON(femb_addr = femb_addr)
                            bl.append(adc_v )
            ...
            if not self.ldflg:
                ...
                if True:
                    for addr in range(0, 0x2B+1,1):
                        val = self.UDP.read_reg_femb(femb_addr, addr)
